...
$(window).on("load", function (){
	...
	if(_tipoWSERP == 'SMEUP' || _tipoWSERP == 'UGOVPA' || _tipoWSERP == 'FNM'){
		_creaTabellaRda();
		_popolaRda();
	}
	...
	$("body").delegate('[id^="ck_rda_"]', "click", function() {
		...
		if (_tipoWSERP == 'SMEUP' || _tipoWSERP == 'UGOVPA' || (_tipoWSERP == 'FNM' && !_multiSelezione)) {
	  ...
	  function _deselezionaTutti() {
		...
		function _popolaRda() {
			...
			if(_tipoWSERP == 'FNM'){
				...
				$('#listarda tfoot td').eq(3).html( '<input class="search" size="20" type="text" placeholder="Ric.società "/>' );
