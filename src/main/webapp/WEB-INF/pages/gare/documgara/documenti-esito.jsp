...
<c:set var="associataRdaWSERP" value='${param.associataRdaWSERP}' />
...
<c:when test="${param.tipoDettaglio eq 1}">
				...
				<gene:campoScheda campo="DIGNOMDOC_${param.contatore}" modificabile="false" entita="W_DOCDIG" campoFittizio="true" definizione="T100;0;;;DIGNOMDOC"  value="${documentiEsitoDescFile[(3 * param.contatore) - 2 ]}" href="javascript:visualizzaFileAllegato('${item[5]}','${item[6]}',${gene:string4Js(documentiEsitoDescFile[(3 * param.contatore) - 2 ])});">
					<c:if test='${modo eq "VISUALIZZA" || !campiEsistentiModificabiliDaProfilo}'>
						<c:if test="${documentiEsitoDescFile[(3 * param.contatore) - 1 ] eq '1'}">
					...
					<c:if test='${not empty documentiEsitoDescFile[(3 * param.contatore) - 2 ] and (empty documentiEsitoDescFile[(3 * param.contatore) - 1 ] or documentiEsitoDescFile[(3 * param.contatore) - 1 ] eq "2") and modo eq "VISUALIZZA" and campiEsistentiModificabiliDaProfilo and param.autorizzatoModifiche ne "2" and param.firmaDocumento eq "1" and item[7] ne "5"}'>
