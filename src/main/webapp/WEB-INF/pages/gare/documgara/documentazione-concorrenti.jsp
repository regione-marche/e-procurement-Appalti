...
<c:choose>
	<c:when test='${busta eq 2}'>
		<c:set var="where" value="GARE.CODGAR1 = '${codiceGara}' and GARE.CODGAR1 != GARE.NGARA and exists (select gare1.ngara from gare1 where gare.ngara=gare1.ngara and (gare1.valtec='1' or gare.modlicg=6))" />
	</c:when>
	<c:otherwise>
		<c:set var="where" value="GARE.CODGAR1 = '${codiceGara}' and GARE.CODGAR1 != GARE.NGARA" />
	</c:otherwise>
</c:choose>
...
<c:when test="${param.tipoDettaglio eq 1}">
				...
				<gene:campoScheda campo="FASELE_${param.contatore}" entita="DOCUMGARA" campoFittizio="true" obbligatorio="true" modificabile="${item[21] ne '1' and campiEsistentiModificabiliDaProfilo and modificabile}" visibile="${(genereGara eq '10' or genereGara eq '20') && tipologia ne '3'}" definizione="N7;0;A1104;;FASELEDG" value="${item[16]}" />
				<c:choose>
				...
				<gene:campoScheda campo="REQCAP_${param.contatore}" entita="DOCUMGARA" campoFittizio="true" visibile="${busta eq 1 or busta eq 4}" modificabile="${item[21] ne '1' and campiEsistentiModificabiliDaProfilo and modificabile}" definizione="N7;0;A1059;;REQCAPDG" value="${item[8]}" />
				<gene:campoScheda campo="TIPODOC_${param.contatore}" entita="DOCUMGARA" campoFittizio="true"  modificabile="${item[21] ne '1' and campiEsistentiModificabiliDaProfilo and modificabile}" definizione="N7;0;A1057;;TIPODOCDG" value="${item[9]}" />
				<gene:campoScheda campo="DESCRIZIONE_${param.contatore}" entita="DOCUMGARA" campoFittizio="true"  modificabile="${item[21] ne '1' and campiEsistentiModificabiliDaProfilo and modificabile}" obbligatorio="true" definizione="T200;0;;;DESCLIBDG" value="${item[13]}" />
				<gene:campoScheda campo="CONTESTOVAL_${param.contatore}" entita="DOCUMGARA" title="Contesto validità ${gene:if(modo eq 'MODIFICA' or modo eq 'NUOVO','(valorizzare solo se documento specifico del tipo di operatore)','') }" campoFittizio="true"  visibile="${item[21] ne '1'}" modificabile="${item[21] ne '1' and campiEsistentiModificabiliDaProfilo and modificabile}" definizione="N7;0;Ag008;;CONTVALDG" value="${item[10]}" />
				<gene:campoScheda campo="OBBLIGATORIO_${param.contatore}" entita="DOCUMGARA" campoFittizio="true"  modificabile="${item[21] ne '1' and campiEsistentiModificabiliDaProfilo and modificabile}" definizione="T2;0;;SN;OBBLIGDG" value="${item[12]}" />
				<gene:campoScheda campo="MODFIRMA_${param.contatore}" entita="DOCUMGARA" campoFittizio="true"  modificabile="${item[21] ne '1' and campiEsistentiModificabiliDaProfilo and modificabile}" definizione="N7;0;A1105;;MODFIRMADG" value="${item[17]}" />
				...
				<c:if test='${(modo eq "MODIFICA" or modo eq "NUOVO") and item[21] ne "1" and campiEsistentiModificabiliDaProfilo and modificabile}'>
...
<c:otherwise>
				...
				<gene:campoScheda campo="FASELE_${param.contatore}" entita="DOCUMGARA" campoFittizio="true" obbligatorio="true" visibile="${(genereGara eq '10' or genereGara eq '20') && tipologia ne '3'}" definizione="N7;0;A1104;;FASELEDG"/>
				<gene:archivio titolo="lotti di gara"
					lista='${gene:if(gene:checkProt(pageContext, "COLS.MOD.GARE.DOCUMGARA.NGARA"),"gare/gare/gare-popup-lista-lotti.jsp","")}'
					scheda=''
					schedaPopUp=''
