...
<c:set var="filtroLivelloUtente" value='${gene:callFunction2("it.eldasoft.gene.tags.utils.functions.FiltroLivelloUtenteFunction", pageContext, "V_GARE_CATALDITTE")}' />
<gene:callFunction obj="it.eldasoft.sil.pg.tags.funzioni.PulisciSessioneFiltriFunction" parametro="listaCataloghi"/>

<c:set var="filtroUffint" value=""/> 
<c:if test="${! empty sessionScope.uffint}">
	<c:set var="filtroUffint" value=" exists (select codgar from torn where codgar = V_GARE_CATALDITTE.CODGAR and CENINT = '${sessionScope.uffint}')"/>
</c:if>


<c:set var="esistonoGarePubblicate"
	value='${gene:callFunction2("it.eldasoft.sil.pg.tags.funzioni.EsistonoGarePubblicateFunction", pageContext,"V_GARE_CATALDITTE")}' />
...
<c:set var="nomeContainerFiltri" value="deftrovaV_GARE_CATALDITTE-${empty param.numeroPopUp ? 0 : param.numeroPopUp}"/> 
...
<gene:template file="lista-template.jsp" gestisciProtezioni="true" schema="GARE" idMaschera="V_GARE_CATALDITTE-lista" >
	<gene:setString name="titoloMaschera" value="Lista cataloghi"/>
	...
	<gene:redefineInsert name="corpo">
  	...
  	<%// Creo la lista per gare e torn mediante la vista %>
		<table class="lista">
			<tr>
				<td>
  	<gene:formLista entita="V_GARE_CATALDITTE" gestisciProtezioni="true" pagesize="20" tableclass="datilista" sortColumn="3" gestore="it.eldasoft.sil.pg.tags.gestori.submit.GestoreElencoDitte"
  		where="${filtroUffint }">
  	...
  	<c:set var='visualizzaPopUp' value='${gene:checkProt(pageContext, "MASC.VIS.GARE.GARE-scheda") || gene:checkProtFunz(pageContext, "MOD","MOD") || gene:checkProtFunz(pageContext, "DEL","DEL") }'/>
				...
				<c:if test='${visualizzaPopUp}'>
					<gene:PopUp variableJs="rigaPopUpMenu${currentRow}" onClick="chiaveRiga='${chiaveRigaJava}'">
						<c:if test='${gene:checkProt(pageContext, "MASC.VIS.GARE.GARE-scheda")}' >
							<gene:PopUpItem title="Visualizza catalogo" href="visualizzaGara('${chiaveRigaJava}')" />
						</c:if>
						<c:if test='${(datiRiga.G_PERMESSI_AUTORI ne "2") && gene:checkProt(pageContext, "MASC.VIS.GARE.GARE-scheda") && gene:checkProtFunz(pageContext, "MOD","MOD")}' >
							<gene:PopUpItem title="Modifica catalogo" href="modificaGara('${chiaveRigaJava}')" />
						</c:if>
						<c:if test='${(datiRiga.G_PERMESSI_AUTORI ne "2") && gene:checkProtFunz(pageContext, "DEL","DEL")}' >
							<gene:PopUpItem title="Elimina catalogo" href="eliminaGara()" />
						</c:if>
						<c:if test='${(abilitazioneGare eq "A" || datiRiga.G_PERMESSI_PROPRI eq "1") && gene:checkProtFunz(pageContext, "ALT","Condividi-catalogo")}'>
							<gene:PopUpItem title="Condividi e proteggi catalogo" href="javascript:apriGestionePermessiStandard('${datiRiga.V_GARE_CATALDITTE_CODGAR}',20,${datiRiga.G_PERMESSI_PROPRI eq '1' or abilitazioneGare eq 'A'})"/>
						</c:if>
						<c:if test='${gene:checkProtFunz(pageContext, "ALT","Archivia-catalogo")}'>
							<c:if test='${(datiRiga.G_PERMESSI_AUTORI ne "2") && (datiRiga.V_GARE_CATALDITTE_ISARCHI ne "1") && gene:checkProtFunz(pageContext, "MOD","MOD")}' >
								<gene:PopUpItem title="Archivia catalogo" href="archiviaGara()" />
							</c:if>
						</c:if>
			...
			<gene:campoLista campo="CODICE"  headerClass="sortable" href="${gene:if(gene:checkProt(pageContext, 'MASC.VIS.GARE.GARE-scheda'), link, '')}" />
			...
			<c:choose>
				<c:when test="${!empty (filtroLivelloUtente)}">
					<gene:campoLista campo="AUTORI" entita="G_PERMESSI" where="G_PERMESSI.CODGAR=V_GARE_CATALDITTE.CODGAR AND G_PERMESSI.SYSCON=${profiloUtente.id}" visibile="false"/>
					<gene:campoLista campo="PROPRI" entita="G_PERMESSI" where="G_PERMESSI.CODGAR=V_GARE_CATALDITTE.CODGAR AND G_PERMESSI.SYSCON=${profiloUtente.id}" visibile="false"/>
				</c:when>
				<c:otherwise>
			...
			<c:if test="${esistonoGarePubblicate == 'SI'}">
				<c:set var="garaPubblicata" value='${gene:callFunction2("it.eldasoft.sil.pg.tags.funzioni.GaraPubblicataSuPortaleFunction",  pageContext, datiRiga.V_GARE_CATALDITTE_CODGAR )}'/>
				<gene:campoLista title="&nbsp;" width="20" >
					<c:if test="${garaPubblicata == 'SI'}">
						<img width="16" height="16" title="Catalogo pubblicato su portale" alt="Catalogo pubblicato su portale" src="${pageContext.request.contextPath}/img/ditta_acquisita.png"/>
					</c:if>
	...
	function eliminaGara(){
		chiaveRiga = chiaveRiga.replace("V_GARE_CATALDITTE", "TORN");
		var href = "href=gare/commons/conferma-eliminazione.jsp&chiaveRiga=" + chiaveRiga + "&numeroPopUp=1&genere=20";
	...
	function archiviaGara(){
		chiaveRiga = chiaveRiga.replace("V_GARE_CATALDITTE", "GAREALBO");
		href = "href=gare/v_gare_torn/v_gare_torn-popup-archivia-gara.jsp&key="+chiaveRiga+ "&isCatalogo=1";
