...
<gene:setString name="titoloMaschera" value='Acquisizione aggiornamento anagrafica da portale Appalti' />
...
<c:set var="modo" value="NUOVO" scope="request" />
	...
	<gene:formScheda entita="IMPR" gestisciProtezioni="false" plugin="it.eldasoft.sil.pg.tags.gestori.plugin.GestoreControlliPreliminariAcquisizioneAggiornamento" gestore="it.eldasoft.sil.pg.tags.gestori.submit.GestorePopupAcquisisciAggiornamento">
		...
		<td colSpan="2">
			...
			<c:choose>
				...
				<c:when test='${not empty requestScope.messaggi}'>
					&nbsp;Sono presenti le seguenti variazioni anagrafiche:
				</c:when>
				<c:otherwise>
					&nbsp;Non vi sono variazioni anagrafiche significative.
				</c:otherwise>
			</c:choose>
	...
	<c:if test='${not empty requestScope.messaggi}'>
		...
		<input type="hidden" name="messaggioVariazioni" id="messaggioVariazioni" value="${requestScope.messaggi}" />
		<input type="hidden" name="impostareStatoNota" id="impostareStatoNota" value="${requestScope.impostareStatoNota}" />
	</gene:formScheda>
  </gene:redefineInsert>
...
<c:if test='${requestScope.erroreAcquisizione eq "1"}' >
	...
	<gene:javaScript>
		...
		function conferma() {
			document.forms[0].jspPathTo.value="gare/commons/popupAcquisizioneAggiornamentoDaPortale.jsp";
			schedaConferma();
		}
