...
<div style="width:97%;">
	...
	<gene:template file="popup-template.jsp">
		...
		<gene:setString name="titoloMaschera" value='Acquisizione busta economica' />
		
		<c:set var="cifraturaBuste" value='${gene:callFunction3("it.eldasoft.sil.pg.tags.funzioni.EsisteBustaCifrataFunction", pageContext, garaBustalotti, "FS11C")}'/>
			...
			<table class="dettaglio-notab">
				<tr id="sezioneIniziale">
					<td class="valore-dato" colspan="2">
						<br>
						Mediante questa funzione &egrave; possibile procedere all'acquisizione della busta economica della ditta '${nomimo}'.
						<br><br>
				...
				<c:if test="${cifraturaBuste eq 'true'}">
						<tr id="rowPWD">
							...
							<td class="valore-dato">
								<input id="PWD" name="PWD" title="Password per la decifratura della busta" class="testo" type="password" size="24" value="${sessionScope.passBusteC }" maxlength="100" />
		...
		<gene:javaScript>
			...
			function inizializzaAvanzamento(ngara,dittao) {
				...
				$.ajax({
					...
					url: "pg/AvanzamentoApEcoAJAX.do?ngara=" + ngara + "&dittao=" + dittao,
			...
			function avanzamento() {
				...
				$.ajax({
					...
					url: "pg/AvanzamentoApEcoAJAX.do?ngara=" + ngara,
					success: function(res){
						var curr = res.cnt;
			...
			function attivaAJAX(){
				...
				$.ajax({
					...
					url: "pg/AttivaApEcoAJAX.do?ngara=" + ngara + "&dittao=" + dittao + "&password=" + encodeURIComponent(pwd) + "&pswCryptata=" + pswCryptata,
					success: function(res){
						esito= res.Esito;
