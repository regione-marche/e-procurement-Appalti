 ...
 */
  ...
  * del wizard Fasi di gara (terzo step) per le gare a lotti con offerta unica.
  * Questa pagina e' stata creata dalla copia della pagina fasiGara-ChiusuraVerificaDocAmm.jsp:
		...
		<td ${stileDati} >
				<gene:formScheda entita="TORN" gestisciProtezioni="true" gestore="it.eldasoft.sil.pg.tags.gestori.submit.GestoreFasiGara_ChiusuraVerificaDocAmm" >
						...
						<c:otherwise>
							...
							<gene:redefineInsert name="addToAzioni">
								<c:if test='${fn:contains(listaOpzioniDisponibili, "OP114#") and autorizzatoModifiche ne 2 
									...
									<c:if test="${isProceduraTelematica eq 'true' and (modalitaAggiudicazioneGara eq '6' or attivaValutazioneTec) and bustalotti ne 1}">
										<c:if test="${faseGara eq '2' or faseGara eq '3' or faseGara eq '4'}">
											<tr>
												<td class="vocemenulaterale">
													<a href="javascript:AttivaAperturaTecniche();" title='Attiva apertura offerte tecniche' tabindex="1504">
								...
								<c:if test='${fn:contains(listaOpzioniDisponibili, "OP114#") and autorizzatoModifiche ne 2 
									...
									and bloccoAggiudicazione ne 1 and gene:checkProt(pageContext, strProtModificaFasiGara)}'>
									<c:if test="${isProceduraTelematica eq 'true' and modalitaAggiudicazioneGara ne '6' and not attivaValutazioneTec and bustalotti ne 1}">
										<c:if test="${faseGara eq '2' or faseGara eq '3' or faseGara eq '4'}">
											<tr>
												<td class="vocemenulaterale">
					...
					<gene:gruppoCampi idProtezioni="VERIFICREQUI">
						...
						<gene:campoScheda campo="DVAPDOCAMM" entita="GARE" where="TORN.CODGAR=GARE.CODGAR1 and TORN.CODGAR=GARE.NGARA" />
						<gene:campoScheda campo="DVVIDONOFF" entita="GARE" where="TORN.CODGAR=GARE.CODGAR1 and TORN.CODGAR=GARE.NGARA" />
						...
						<gene:campoScheda campo="DGARA" visibile="${datiRiga.TORN_COMPREQ eq '1' }" />
						<gene:campoScheda campo="OGARA" visibile="${datiRiga.TORN_COMPREQ eq '1' }" />
					</gene:gruppoCampi>

					
					
					<gene:campoScheda campo="COMPREQ" visibile="false" />
