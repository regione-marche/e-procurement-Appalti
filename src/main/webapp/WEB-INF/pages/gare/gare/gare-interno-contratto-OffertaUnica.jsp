...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<gene:gruppoCampi idProtezioni="CAUZIONE" visibile="${param.aqoper ne '2' }">
		...
		<gene:campoScheda title="Importo garanzia definitiva complessiva" campo="CAUZIONE" campoFittizio="true" definizione="F15.5;;;MONEY" modificabile="false" value="${importoCauzione}" visibile="${param.modcont eq 2 }"/>
		<gene:campoScheda  campo="IMPGAR" visibile="${param.modcont eq 1 }" modificabile="${empty datiRiga.G1STIPULA_ID}"/>
		...
		<gene:campoScheda campo="DSCAPO" entita="GARECONT" where="GARECONT.NGARA=GARE.CODGAR1 AND GARECONT.NCONT=${param.ncont }" modificabile="${empty datiRiga.G1STIPULA_ID}"/>
	...
	<gene:campoScheda campo="ID" entita="G1STIPULA" where="G1STIPULA.NGARA=GARE.CODGAR1 AND NCONT=${param.ncont } AND ID_PADRE IS NULL" visibile="false"/>
	<gene:gruppoCampi idProtezioni="STIPCONT" visibile="${param.chiamante ne 'aggiudicazione-efficace' && !(empty datiRiga.G1STIPULA_ID) }">
		...
		<gene:campoScheda campo="CODSTIPULA" entita="G1STIPULA" where="G1STIPULA.NGARA=GARE.CODGAR1 AND NCONT=${param.ncont } AND ID_PADRE IS NULL" modificabile="false" />
		<gene:campoScheda campo="OGGETTO" entita="G1STIPULA" where="G1STIPULA.NGARA=GARE.CODGAR1 AND NCONT=${param.ncont } AND ID_PADRE IS NULL" modificabile="false" />
		<gene:campoScheda campo="IMPSTIPULA" entita="G1STIPULA" where="G1STIPULA.NGARA=GARE.CODGAR1 AND NCONT=${param.ncont } AND ID_PADRE IS NULL" modificabile="false" />
		<gene:campoScheda campo="SYSCON" entita="G1STIPULA" where="G1STIPULA.NGARA=GARE.CODGAR1 AND NCONT=${param.ncont } AND ID_PADRE IS NULL" visibile="false" />
		...
		<gene:campoScheda campo="ASSEGNATARIO" entita="G1STIPULA" where="G1STIPULA.NGARA=GARE.CODGAR1 AND NCONT=${param.ncont } AND ID_PADRE IS NULL" visibile="false" />
		...
		<gene:campoScheda campo="STATO" entita="G1STIPULA" where="G1STIPULA.NGARA=GARE.CODGAR1 AND NCONT=${param.ncont } AND ID_PADRE IS NULL" modificabile="false" />
	...
	<gene:gruppoCampi idProtezioni="ATTOCONTR" >
		...
		<gene:campoScheda campo="TIATTO" visibile="${param.tipoContratto eq 'contratto'}" modificabile="${empty datiRiga.G1STIPULA_ID}"/>
		<gene:campoScheda campo="NREPAT" visibile="${param.tipoContratto eq 'contratto'}" modificabile="${empty datiRiga.G1STIPULA_ID}"/>
		...
		<gene:campoScheda campo="NUMRDO" entita="GARE1" where="GARE1.NGARA=GARE.NGARA" title="Invio dati contratto ERP" visibile='${tipoWSERP eq "CAV" or tipoWSERP eq "AMIU"}' modificabile='false' />
		<gene:campoScheda campo="DAATTO" visibile="${param.tipoContratto ne 'aggEff' }" modificabile="${empty datiRiga.G1STIPULA_ID}"/>
		...
		<gene:campoScheda campo="DPROAT" entita="GARECONT" where="GARECONT.NGARA=GARE.CODGAR1 AND GARECONT.NCONT=${param.ncont }" visibile="${param.tipoContratto eq 'contratto'}"/>
	...
	<c:if test="${res eq 1}">
		<jsp:include page="/WEB-INF/pages/commons/interno-scheda-multipla.jsp" >
			...
			<jsp:param name="chiave" value='${param.codiceGara}'/>
...
<gene:javaScript>
			...
			var opt = {
				...
				buttons: {
					"Conferma": {
						...
						text:"Conferma",
						click: function() {
							generaNRepat(genere,ngara,modcont);
						}
					 },
					"Annulla": {
						id:"botAnnulla",
		...
		function generaNRepat(genere,ngara,modcont){
			_wait();
			$.ajax({
				type: "GET",
				dataType: "text",
