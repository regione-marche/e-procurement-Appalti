...
public class AttivaApDocAJAXAction extends Action {

  static Logger                 logger          = Logger.getLogger(AttivaApDocAJAXAction.class);
  ...
  public final ActionForward execute(final ActionMapping mapping, final ActionForm form, final HttpServletRequest request,
    ...
    logger.info("Apertura busta amministrativa: inizio");
    ...
    String garaInversa = request.getParameter("garaInversa");
    String comtipo = "FS11A";
    Long fasgarSuccessivo = new Long(2);
    Long stepgarSuccessivo = new Long(20);
    ...
    try{
      ...
      try {
        ...
        logEvento.setCodEvento("GA_APERTURA_BUSTA_AMM_INIT");
        logEvento.setDescr("Inizio apertura busta amministrativa");
        logEvento.setErrmsg("");
      ...
      if (!this.geneManager.getProfili().checkProtec(
          ...
          "ALT.GARE.FASIGARA.AcquisisciDocAmm") || !this.pgManager.getAutorizzatoModificaGara("V_GARE_TORN",
              "CODGAR", ngara, false, "2", request)
          || (!"1".equals(garaInversa) && this.mepaManager.esisteBloccoCondizioniFasiAcquisizioni(ngara, new Long(2)) && this.mepaManager.esisteBloccoCondizioniFasiAcquisizioni(ngara, new Long(3)) && this.mepaManager.esisteBloccoCondizioniFasiAcquisizioni(ngara, new Long(4)))) {
        erroreGestito=true;
        messageKey ="errors.gestoreException.*.acquisizione.BustaAmm.DatiIncosistenti";
      ...
      if(!erroreGestito){
        esitoControllo = mepaManager.controlloDataConDataAttuale(ngara,"dteoff","oteoff");
        if("false".equals(esitoControllo[0])){
          erroreGestito=true;
          messageKey ="errors.gestoreException.*.acquisizione.BustaAmm.TerminiOfferteNonScaduti";
          messaggioPerLog = resBundleGenerale.getString(messageKey);
      ...
      //Controllo che la data termine apertura plichi non sia passata
      if(!erroreGestito){
        esitoControllo =  mepaManager.controlloDataConDataAttuale(ngara,"desoff","oesoff");
        if("false".equals(esitoControllo[0])){
          erroreGestito=true;
          messageKey ="errors.gestoreException.*.acquisizione.BustaAmm.TerminiPlichiNonScaduti";
          messaggioPerLog = resBundleGenerale.getString(messageKey);
          messaggioPerLog = messaggioPerLog.replace("{0}", esitoControllo[1]);
          messaggioPerLog = messaggioPerLog.replace("{1}", esitoControllo[2]);
          errMsgEvento = messaggioPerLog;
      ...
      //Controllo che ci siano ditte che hanno presentato offerte
      if(!erroreGestito){
        if(!mepaManager.esistonoDittePresentataOfferta(ngara)){
          erroreGestito=true;
          messageKey ="errors.gestoreException.*.acquisizione.BustaAmm.NessunaDitta";
          messaggioPerLog = resBundleGenerale.getString(messageKey);
          errMsgEvento = messaggioPerLog;
          result.put("Esito", "Errore");
          result.put("MsgErrore", errMsgEvento);
      ...
      if(!erroreGestito){
        ...
        request.getSession().setAttribute("passBusteA", password);
      ...
      try {
        ...
        logEvento.setOggEvento(ngara);
        logEvento.setCodEvento("GA_APERTURA_BUSTA_AMM_FINE");
        logEvento.setDescr("Fine apertura busta amministrativa: " +
      ...
      logger.info("Apertura busta amministrativa (gara "+ngara+"): numero acquisizioni " + numeroAcquisizioni);
      logger.info("Apertura busta amministrativa (gara "+ngara+"): numero acquisizioni con errore " + numeroAcquisizioniErrore);
      logger.info("Apertura busta amministrativa (gara "+ngara+"): non acquisite perchè escluse dalla gara " + numeroAcquisizioniScartate);
    ...
    logger.info("Apertura busta amministrativa: fine");
