...
/**
 * Action per la visualizzazione della scheda con i permessi sulla gara degli utenti. 
 * 
 * @author Luca.Giacomazzo
 */
public class VisualizzaPermessiUtentiStandardAction extends DispatchActionBaseNoOpzioni {

  protected final String FORWARD_VISUALIZZA = "visualizza";

  static Logger                 logger             = Logger.getLogger(VisualizzaPermessiUtentiStandardAction.class);
	...
	public ActionForward apri(ActionMapping mapping, ActionForm form,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("VisualizzaPermessiUtentiStandardAction: inizio metodo");
		}
		...
		try {
			...
			// Si determina se l'entità di cui si stanno visualizzando i
			// permessi possiede una condivisione predefinita o meno
		...
		if (logger.isDebugEnabled()) {
			logger.debug("VisualizzaPermessiUtentiStandardAction: fine metodo");
		}
  ...
  /**
	...
	public ActionForward setPermessiPredefiniti(ActionMapping mapping,
		...
		try {
			...
			List<?> listaPermessiEntita = this.permessiManager.getListaPermessiEntita("CODGAR", valoreChiave);
					...
					try {
						this.permessiManager.insertPermessiPredefiniti("codgar",valoreChiave,
							new Integer(utenteDiRiferimento),
								CostantiPermessi.VALORE_DEFAULT_PREDEFINITO_GARE,
									listaPermessiEntita);
						inserito = true;
