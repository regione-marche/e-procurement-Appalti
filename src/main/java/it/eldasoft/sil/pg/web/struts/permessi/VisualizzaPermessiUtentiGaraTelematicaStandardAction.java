...
/**
 * Action per la visualizzazione della scheda con i permessi sulla gara telematica degli utenti.
 * 
 * @author Luca.Giacomazzo
 */
public class VisualizzaPermessiUtentiGaraTelematicaStandardAction extends DispatchActionBaseNoOpzioni {
	
	protected final String FORWARD_VISUALIZZA = "visualizza";

	static Logger          logger             = Logger.getLogger(VisualizzaPermessiUtentiGaraTelematicaStandardAction.class);
	...
	public ActionForward apri(ActionMapping mapping, ActionForm form,
	    ...
	    if (logger.isDebugEnabled()) {
	    	logger.debug("VisualizzaPermessiUtentiGaraTelematicaAction: inizio metodo");
	    }
		...
		if (logger.isDebugEnabled()) {
			logger.debug("VisualizzaPermessiUtentiGaraTelematicaAction: fine metodo");
		}
	...
	public ActionForward setPermessiPredefiniti(ActionMapping mapping,
		...
		try {
			...
			int utenteDiRiferimento = ((ProfiloUtente) 
			...
			if (listaPermessiEntita.size() > 0) {
				...
				while (!inserito
					...
					try {
						this.permessiManager.insertPermessiPredefiniti(
							"CODGAR", valoreChiave, new Integer(utenteDiRiferimento),
