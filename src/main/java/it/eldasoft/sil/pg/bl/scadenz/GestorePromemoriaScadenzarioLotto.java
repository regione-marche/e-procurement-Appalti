...
public class GestorePromemoriaScadenzarioLotto extends AbstractGestorePromemoriaScadenzario{

  @Override
  public String getModello() {
    return "promemoria-scadenzario-lotto.txt";
  }
  ...
  public void popolaContesto(String codapp, Long idAttivita, String ent, Object[] chiavi, VelocityContext velocityContext)
    ...
    //Nel campo chiave[0] è presente sempre il valore di NGARA del lotto
    String codgar=null;
    String oggettoLotto=null;
    String codiceCig=null;
    Vector<JdbcParametro> datiLottoGara = this.sqlManager.getVector(
        "select codgar1,not_gar,codcig from gare where ngara=?", new Object[]{chiavi[0]});
    if(datiLottoGara!=null && datiLottoGara.size()>0){
      codgar = (String) (datiLottoGara.get(0)).getValue();
      oggettoLotto = (String) (datiLottoGara.get(1)).getValue();
      codiceCig = (String) (datiLottoGara.get(2)).getValue();
    }
    ...
    Vector<JdbcParametro> datiGara = this.sqlManager.getVector(
        "select genere,codice,tipgen,oggetto from v_gare_torn where codgar=?", new Object[]{codgar});
    ...
    velocityContext.put("CODICE_LOTTO", chiavi[0]);
    velocityContext.put("OGGETTO_LOTTO", oggettoLotto);
    velocityContext.put("CODCIG", codiceCig);
