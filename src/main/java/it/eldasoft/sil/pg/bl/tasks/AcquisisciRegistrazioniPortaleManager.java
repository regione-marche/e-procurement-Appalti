...
public class AcquisisciRegistrazioniPortaleManager {
   ...
   */
  public void acquisisciRegistrazioni() throws GestoreException {
    ...
    if (logger.isDebugEnabled())
      logger.debug("acquisisciRegistrazioni: inizio metodo");
    ...
    try {
      listaIDCOM = sqlManager.getListVector(select,
            new Object[] { idprg, comstato,comtipo});
    ...
    if (listaIDCOM != null && listaIDCOM.size() > 0) {
      ...
      for (int i = 0; i < listaIDCOM.size(); i++) {
        ...
        try{
          ...
          if (errore)
            pgManager.aggiornaStatoW_INVOCM(idcom,"7","");
    ...
    if (logger.isDebugEnabled())
      logger.debug("acquisisciRegistrazioni: fine metodo");
