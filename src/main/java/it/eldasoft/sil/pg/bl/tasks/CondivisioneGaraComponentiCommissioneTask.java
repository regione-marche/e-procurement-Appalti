...
public class CondivisioneGaraComponentiCommissioneTask {
  ...
  public void setPermessiComponentiCommisione() throws GestoreException {
    ...
    String selectPermessi = "select usrsys.syscon from tecni, usrsys where tecni.codtec  = ? and upper(tecni.cftec) = upper(usrsys.syscf) and (usrsys.sysdisab is null or usrsys.sysdisab = '0')";
    ...
    try {
      ...
      if (listaTecnici != null && listaTecnici.size() > 0) {
        for (int i = 0; i < listaTecnici.size(); i++) {
          ...
          String permessiAttribuitiCount = "select count(*) from usrsys,tecni,g_permessi where tecni.codtec = ? and upper(usrsys.syscf) = upper(tecni.cftec) and" +
          ...
          if(cntPermesssi.intValue()<=0){
            ...
            if (listaUtenti != null && listaUtenti.size() == 1) {
                Long syscon = SqlManager.getValueFromVectorParam(listaUtenti.get(0), 0).longValue();
